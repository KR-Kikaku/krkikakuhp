...
<p style="margin-top: 3rem; text-align: right; color: #6b7280;">制定日：2025年12月23日<br>合同会社 KR企画</p>
  ...
  const handleSave = async () => {
    ...
    try {
      let result;
      if (settingsId) {
        result = await base44.entities.SiteSettings.update(settingsId, {
          privacy_policy: settings.privacy_policy
        });
      } else {
        result = await base44.entities.SiteSettings.create({
          privacy_policy: settings.privacy_policy
        });
      ...
      toast.success('プライバシーポリシーを保存しました');
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <AdminLayout currentPage="privacy">
    ...
    <AdminLayout currentPage="privacy">
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">プライバシーポリシーページの内容を管理</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>プライバシーポリシーの内容</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-4">
